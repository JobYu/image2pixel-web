...
(function () {
    ...
    const messages = {
        en: {
            ...
            'algo.classic': 'Classic (Median Cut)',
            'algo.classicLab': 'Classic LAB ✦',
            'algo.labDither': 'LAB + Dithering',
            'algo.tezumie': 'Tezumie Style',
            'algo.tezumieNoDither': 'Tezumie (No Dither)',
            ...
            'content.algorithmTitle': 'Conversion Algorithms',
            'content.algoClassicTitle': 'Classic (Median Cut)',
            'content.algoClassicBody': 'The original algorithm. Averages all pixels in each block, generates a palette via Median Cut in <strong>RGB space</strong>, and maps each block to the nearest color. Fast and predictable — kept as a comparison baseline.',
            'content.algoLabTitle': 'Classic LAB ✦ (Recommended)',
            'content.algoLabBody': 'Same clean solid-color blocks as Classic, with two key upgrades: palette generation runs in <strong>CIELAB color space</strong> for more perceptually distinct colors, and color matching uses <strong>LAB distance</strong> — the nearest color is the one that <em>looks</em> closest to the human eye, not just the one closest in RGB math. Best choice for authentic pixel art.',
            'content.algoDitherTitle': 'LAB + Dithering',
            'content.algoDitherBody': 'Adds <strong>Floyd-Steinberg dithering</strong> on top of the LAB pipeline. Color error from each block is diffused to neighbors in a 7/16 · 5/16 · 3/16 · 1/16 pattern, creating smooth gradients with a larger effective palette. Well suited for images with rich color transitions.',
            'content.algoTezumieTitle': 'Tezumie Style',
            'content.algoTezumieBody': 'Inspired by the open-source <em>Tezumie/Image-to-Pixel</em> library. Uses <strong>nearest-neighbor center-pixel sampling</strong> instead of block averaging — preserving hard edges — then applies LAB Floyd-Steinberg dithering. Produces sharper outlines with a characteristic dithered texture.',
        ...
        'zh-CN': {
            ...
            'control.algorithm': '算法：',
            'control.openImage': '打开图片',
            'algo.classic': '经典 (Median Cut)',
            'algo.classicLab': '经典 LAB ✦',
            'algo.labDither': 'LAB + 抖动',
            'algo.tezumie': 'Tezumie 风格',
            'algo.tezumieNoDither': 'Tezumie (无抖动)',
            'preview.original': '原图',
            ...
            'content.algorithmTitle': '转换算法',
            'content.algoClassicTitle': '经典 (Median Cut)',
            'content.algoClassicBody': '原始算法。对每个色块取平均像素值，在 <strong>RGB 空间</strong>用 Median Cut 生成调色板，再将每个色块映射到最近颜色。速度快、结果稳定，作为对比基准。',
            'content.algoLabTitle': '经典 LAB ✦（推荐）',
            'content.algoLabBody': '与经典模式同样产生干净的固体色块，但有两项关键改进：调色板生成在 <strong>CIELAB 色彩空间</strong>中运行，颜色分布更均匀；颜色匹配采用 <strong>LAB 距离</strong>而非 RGB——找的是视觉上最接近的颜色，而非 RGB 数值最近的颜色。最适合正统像素画风格。',
            'content.algoDitherTitle': 'LAB + 抖动',
            'content.algoDitherBody': '在 LAB 管道基础上加入 <strong>Floyd-Steinberg 抖动</strong>。每个色块的颜色误差按 7/16 · 5/16 · 3/16 · 1/16 比例扩散至相邻色块，颜色过渡更平滑，等效色彩数大幅增加。适合渐变丰富的图像。',
            'content.algoTezumieTitle': 'Tezumie 风格',
            'content.algoTezumieBody': '灵感来自开源库 <em>Tezumie/Image-to-Pixel</em>。使用<strong>最近邻中心采样</strong>代替区域平均——保留硬边缘——再结合 LAB Floyd-Steinberg 抖动处理颜色。输出轮廓更锐利，带有独特的抖动纹理感。',
            'faq.title': '常见问题',
            'faq.q1': '这个工具免费吗？',
            'faq.a1':
        ...
        'zh-TW': {
            ...
            'control.algorithm': '演算法：',
            'control.openImage': '開啟圖片',
            'algo.classic': '經典 (Median Cut)',
            'algo.classicLab': '經典 LAB ✦',
            'algo.labDither': 'LAB + 抖動',
            'algo.tezumie': 'Tezumie 風格',
            'algo.tezumieNoDither': 'Tezumie (無抖動)',
            'preview.original': '原圖',
            ...
            'content.algorithmTitle': '轉換演算法',
            'content.algoClassicTitle': '經典 (Median Cut)',
            'content.algoClassicBody': '原始演算法。對每個色塊取平均像素值，在 <strong>RGB 空間</strong>以 Median Cut 產生調色盤，再將每個色塊映射至最近顏色。速度快、結果穩定，作為對比基準。',
            'content.algoLabTitle': '經典 LAB ✦（推薦）',
            'content.algoLabBody': '與經典模式同樣產生乾淨的固體色塊，但有兩項關鍵改進：調色盤生成在 <strong>CIELAB 色彩空間</strong>中執行，顏色分布更均勻；顏色比對採用 <strong>LAB 距離</strong>而非 RGB——找的是視覺上最接近的顏色，而非 RGB 數值最近的顏色。最適合正統像素畫風格。',
            'content.algoDitherTitle': 'LAB + 抖動',
            'content.algoDitherBody': '在 LAB 管線基礎上加入 <strong>Floyd-Steinberg 抖動</strong>。每個色塊的顏色誤差按 7/16 · 5/16 · 3/16 · 1/16 比例擴散至相鄰色塊，顏色過渡更平滑，等效色彩數大幅增加。適合漸層豐富的圖像。',
            'content.algoTezumieTitle': 'Tezumie 風格',
            'content.algoTezumieBody': '靈感來自開源函式庫 <em>Tezumie/Image-to-Pixel</em>。使用<strong>最近鄰中心採樣</strong>取代區域平均——保留硬邊緣——再結合 LAB Floyd-Steinberg 抖動處理顏色。輸出輪廓更銳利，帶有獨特的抖動紋理感。',
            'faq.title': '常見問題',
            'faq.q1': '這個工具免費嗎？',
            'faq.a1':
